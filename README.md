...
- Updates `armbianEnv.txt` — adds `fdtfile`, `spi-spidev` and `sun8i-h3-spi0`
- Enables `spidev` kernel module on boot
- Replaces plugin files with NanoPi NEO compatible versions:
  - `gpiomap.js` — hardcoded GPIO mapping (fixes 77/NOPERM error)
...
| CLK         | Pin 23 (SPI0_CLK, PC2)     | SPI clock     |
| MOSI        | Pin 19 (SPI0_MOSI, PC0)    | SPI data      |
| CS          | Pin 24 (SPI0_CS, PC3)      | Chip select   |
...
- `sun8i-h3-i2s0-slave.dtbo` is **modified** — `spi0_cs_pins` block removed to free SPI0_CS pin (PC3). Without this fix `/dev/spidev0.0` does not appear.
...
| Screen goes to clock during Spotify | seek not updated via WebSocket | Fixed in `volumiolistener.js` |
